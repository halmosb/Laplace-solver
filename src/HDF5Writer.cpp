...
void HDF5Writer::writeResult(const boost::multi_array<float, 2>& result) {
    ...
    H5::DataSet dataset = file_.createDataSet("result", H5::PredType::NATIVE_FLOAT, dataspace);
...
void HDF5Writer::writeAttribute(const std::string& name, int value) {
    ...
    H5::Attribute attr = paramGroup_.createAttribute(name, H5::PredType::NATIVE_INT, dataspace);
    attr.write(H5::PredType::NATIVE_INT, &value);
}

void HDF5Writer::writeAttribute(const std::string& name, float value) {
    H5::DataSpace dataspace(H5S_SCALAR);
    H5::Attribute attr = paramGroup_.createAttribute(name, H5::PredType::NATIVE_FLOAT, dataspace);
